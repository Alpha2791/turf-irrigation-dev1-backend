...
def get_wilt_forecast(wilt_point: float = 18.0, upper_limit: float = 22.0):
    ...
    db = SessionLocal()
    df_irrig = pd.DataFrame([{"timestamp": e.timestamp, "irrigation_mm": e.irrigation_mm} for e in db.query(IrrigationLog).all()])
    df_moist = pd.DataFrame([{"timestamp": e.timestamp, "moisture_mm": e.moisture_mm} for e in entries])
    db.close()
